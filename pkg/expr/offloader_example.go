...
// NamespaceOffloader is an example of a string-based offloader for namespace filtering
			...
			case *SetConstraint:
				...
				return handler.ActivateStringSetConstraint(ctx, constraint,
					func(ctx context.Context, values []string) (bool, error) {
						slog.LogAttrs(ctx, slog.LevelDebug, "Activating namespace filter for multiple values",
							slog.Any("namespaces", values))
...
// PortOffloader is an example of a port number offloader with range support
		...
		CheckCallback: func(c any) (bool, error) {
			...
			case *EqualsConstraint:
				...
				if portValue < 1 || portValue > 65535 {
					// Port value outside valid range (1-65535)
					return false, nil
				}
			...
			case *EqualsConstraint:
				return handler.ActivateNumericEqualsConstraint(ctx, constraint,
					func(ctx context.Context, value int64) (bool, error) {
						slog.LogAttrs(ctx, slog.LevelDebug, "Activating port filter",
			...
			case *RangeConstraint:
				return handler.ActivateNumericRangeConstraint(ctx, constraint,
					func(ctx context.Context, min, max *int64) (bool, error) {
						...
						slog.LogAttrs(ctx, slog.LevelDebug, "Activating port range filter",
							slog.Int64("min", minPort),
			...
			case *SetConstraint:
				return handler.ActivateNumericSetConstraint(ctx, constraint,
					func(ctx context.Context, values []int64) (bool, error) {
						slog.LogAttrs(ctx, slog.LevelDebug, "Activating port filter for multiple values",
							slog.Any("ports", values))

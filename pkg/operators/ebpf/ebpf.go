...
func (i *ebpfInstance) Prepare(gadgetCtx operators.GadgetContext) error {
	...
	if len(i.tcHandlers) > 0 {
		...
		gadgetCtx.SetVar("overrideEnricher", true)
...
func (i *ebpfInstance) Start(gadgetCtx operators.GadgetContext) error {
	...
	parms := params.Params{}                   // used to CopyFromMap
	...
	for name, p := range i.params {
		...
		paramMap[name] = param
		parms = append(parms, param)
	}
	err := parms.CopyFromMap(i.paramValues, "")
	if err != nil {
	...
	for name, p := range i.params {
		...
		gadgetCtx.Logger().Debugf("setting param value %q = %v", name, paramMap[name].AsAny())
	}

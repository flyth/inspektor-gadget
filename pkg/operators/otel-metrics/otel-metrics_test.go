...
func TestMetricsCounterAndGauge(
	t *testing.T,
) {
...
func TestMetricsHistogram(
	...
	prepare := func(gadgetCtx operators.GadgetContext) error {
		...
		value, err = ds.AddField("duration", api.Kind_Uint32, datasource.WithAnnotations(map[string]string{
			"metrics.type": MetricTypeHistogram,
		}))
		assert.NoError(t, err)
		return nil
	}
		...
		for _, m := range sm.Metrics {
			if m.Name == "duration" {
				...
				for i, v := range expectedBuckets {
					assert.Equal(t, v, data.DataPoints[0].BucketCounts[i])
				}
			}
		}
		assert.True(t, found)

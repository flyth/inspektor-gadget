...
func (l *KubeManagerInstance) enrich(ev any) {
	if event, canEnrichEventFromMountNs := ev.(operators.ContainerInfoFromMountNSID); canEnrichEventFromMountNs {
		l.manager.gadgetTracerManager.ContainerCollection.EnrichEventByMntNs(event)
	}
	if event, canEnrichEventFromNetNs := ev.(operators.ContainerInfoFromNetNSID); canEnrichEventFromNetNs {
		l.manager.gadgetTracerManager.ContainerCollection.EnrichEventByNetNs(event)
	}
}

func (m *KubeManagerInstance) Enricher(next operators.EnricherFunc) operators.EnricherFunc {
	if !m.enrichEvents {
		return nil
	}

...
var ellipsisTests = []ellipsisTest{
	{
		Input:     "Demo",
		MaxLength: 8,
		Type:      None,
...
func TestEllipsis(t *testing.T) {
	for _, test := range ellipsisTests {
		t.Run(fmt.Sprintf("%s_len%d_type%d", test.Input, test.MaxLength, test.Type), func(t *testing.T) {
			if res := ShortenString([]rune(test.Input), test.MaxLength, test.Type); string(res) != test.Result {
...
func FuzzEllipsis(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, inputString string, maxLength int) {
		...
		if out := ShortenString([]rune(inputString), maxLength, None); len(out) > correctedLength {
			t.Errorf("None of %q (%d): wrong output %q of length %d", inputString, maxLength, out, len(out))
		}
		if out := ShortenString([]rune(inputString), maxLength, End); len(out) > correctedLength {
			t.Errorf("End of %q (%d): wrong output %q of length %d", inputString, maxLength, out, len(out))
		}
		if out := ShortenString([]rune(inputString), maxLength, Start); len(out) > correctedLength {
			t.Errorf("Start of %q (%d): wrong output %q of length %d", inputString, maxLength, out, len(out))
		}
		if out := ShortenString([]rune(inputString), maxLength, Middle); len(out) > correctedLength {

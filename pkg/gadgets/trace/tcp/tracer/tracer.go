...
func (t *Tracer) run() {
	for {
		...
		eventC := (*tcptracerEvent)(unsafe.Pointer(&record.RawSample[0]))
		...
		event := types.Event{
			...
			Dport:     gadgets.Htons(eventC.Dport),
			Sport:     gadgets.Htons(eventC.Sport),
		}

		if eventC.Af == unix.AF_INET {
			event.IPVersion = 4
			event.Saddr = netip.AddrFrom4(*(*[4]byte)(eventC.SaddrV6[:4])).String()
			event.Daddr = netip.AddrFrom4(*(*[4]byte)(eventC.DaddrV6[:4])).String()
		} else if eventC.Af == unix.AF_INET6 {
			event.IPVersion = 6
			event.Saddr = netip.AddrFrom16(eventC.SaddrV6).String()
			event.Daddr = netip.AddrFrom16(eventC.DaddrV6).String()
		}

		if eventC.Type == tcptracerEventTypeTCP_EVENT_TYPE_CONNECT {
			event.Operation = "connect"
		} else if eventC.Type == tcptracerEventTypeTCP_EVENT_TYPE_ACCEPT {
			event.Operation = "accept"
		} else if eventC.Type == tcptracerEventTypeTCP_EVENT_TYPE_CLOSE {

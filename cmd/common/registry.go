...
func buildCommandFromGadget(gadget gadgets.Gadget,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if parser != nil {
				...
				if gadget.Type().CanSort() {
					err := parser.SetSorting(strings.Split(gadgetParams.Get(gadgets.ParamSortBy), ","))
					if err != nil {
						return err // TODO: Wrap
					}
				}
				...
				default:
					...
					if gadget.Type().IsPeriodic() {
						...
						runner.Parser().SetEventCallbackArray(formatter.EventHandlerFuncArray(
							fe.Clear,
							func() {
				...
				case utils.OutputModeJSON:
					...
					runner.Parser().SetEventCallbackArray(printEventAsJSON)
				}
			} else {
				defer func() {
	...
	gadgetParams.Add(gadgets.GadgetParams(gadget, parser)...)
